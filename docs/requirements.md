...
**重要な変更**: 以前のshared/index.tsによる一元管理から、フロントエンドとバックエンドそれぞれに型定義ファイルを配置する方式に変更されました。
...
1. **変更の責任**
   ...
   - 変更時は必ず両ファイルの差分を確認し、完全に一致していることを保証する
...
3. **同期の確認方法**
   - コミット前に`diff frontend/src/types/index.ts backend/src/types/index.ts`を実行
   - 差分がある場合は必ず同期してからコミット
   - TypeScriptエラーゼロマネージャーのツールを活用して型の不整合を検出
