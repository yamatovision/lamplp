...
exports.PromptLibraryPanel = void 0;
const vscode = __importStar(require("vscode"));
const claudeCodeApiClient_1 = require("../../api/claudeCodeApiClient");
const CategoryManager_1 = require("./CategoryManager");
const PromptEditor_1 = require("./PromptEditor");
const PromptImportExport_1 = require("./PromptImportExport");
/**
 * プロンプトライブラリパネルクラス
 * - プロンプト一覧の表示
 * - カテゴリによるフィルタリング
...
class PromptLibraryPanel {
    ...
    static createOrShow(extensionUri, options) {
        ...
        // 新しいパネルを作成
        const panel = vscode.window.createWebviewPanel('promptLibrary', 'プロンプトライブラリ', column || vscode.ViewColumn.One, {
    ...
    constructor(panel, extensionUri, options) {
        ...
        this._categoryManager = new CategoryManager_1.CategoryManager();
        this._promptEditor = new PromptEditor_1.PromptEditor();
        this._promptImportExport = new PromptImportExport_1.PromptImportExport();
        // webviewの内容を設定
     ...
     * エディタモードの変更を処理
     */
    async _handleEditorModeChange(options) {
        this._setLoading(true);
        try {
    ...
    async _loadPrompts() {
        ...
        try {
            ...
            if (this._searchQuery) {
                ...
                filteredPrompts = this._currentPrompts.filter(p => p.title.toLowerCase().includes(query) ||
    ...
    _getHtmlForWebview(extensionUri) {
        ...
        const styleUri = this._panel.webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'webviews', 'promptLibrary', 'style.css'));
        const scriptUri = this._panel.webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'webviews', 'promptLibrary', 'script.js'));
        // コンテンツセキュリティポリシーの設定
...
exports.PromptLibraryPanel = PromptLibraryPanel;
//# sourceMappingURL=PromptLibraryPanel.js.map