...
exports.AIDevelopmentService = void 0;
...
各フェーズでの質問や指示に対して、具体的かつ明確に回答してください。`;
    ...
    updateSystemPrompt(newPrompt) {
        ...
        logger_1.Logger.info('開発アシスタントのシステムプロンプトを更新しました');
    }
    /**
     * 開発関連のメッセージを処理する
     */
    async processMessage(message, chatHistory) {
        try {
            ...
            logger_1.Logger.error('開発アシスタントのメッセージ処理エラー:', error);
     ...
     * ストリーミングモードでメッセージを処理
     */
    async processMessageWithStreaming(message, onChunk, onComplete, chatHistory) {
        try {
            // チャット履歴がない場合は新しく作成
            const history = chatHistory || [];
            // システムプロンプトがまだ履歴に含まれていない場合は追加
            ...
            // ストリーミングモードでメッセージを処理
            await this._aiService.sendMessageWithStreaming(message, 'development', (chunk) => {
                onChunk(chunk);
            }, (fullResponse) => {
                // アシスタントの応答を履歴に追加
                history.push({
            ...
            logger_1.Logger.error('開発アシスタントのストリーミングメッセージ処理エラー:', error);
...
実装を開始するにあたり、必要な情報はすべて揃っていますか？`;
     ...
     * コード生成用の特化したプロンプトで処理
     */
    async generateCode(description, context, language) {
        try {
...
コードは完全に機能する必要があり、エラー処理やベストプラクティスに準拠してください。`;
            ...
            logger_1.Logger.error('コード生成エラー:', error);
            return `コード生成中にエラーが発生しました: ${error.message}`;
        }
    }
}
exports.AIDevelopmentService = AIDevelopmentService;
//# sourceMappingURL=aiDevelopmentService.js.map