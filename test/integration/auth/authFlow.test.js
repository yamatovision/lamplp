...
const ClaudeCodeAuthSync_1 = require("../../../src/services/ClaudeCodeAuthSync");
// モック化のためのヘルパー
...
async function setupTestEnvironment() {
    ...
    // インスタンスの初期化
    authService = AuthenticationService_1.AuthenticationService.getInstance();
    authEventBus = AuthEventBus_1.AuthEventBus.getInstance();
    claudeCodeSync = ClaudeCodeAuthSync_1.ClaudeCodeAuthSync.getInstance();
    // fs操作のモック
    const fs = require('fs');
...
suite('認証フロー統合テスト', () => {
    ...
    test('E2E認証フロー - ログイン→状態同期→ログアウト', async () => {
        ...
        // 認証イベントバスの監視
        authEventBus.on(AuthEventBus_1.AuthEventType.LOGIN_SUCCESS, loginEventSpy);
        authEventBus.on(AuthEventBus_1.AuthEventType.LOGOUT, logoutEventSpy);
        authEventBus.on(AuthEventBus_1.AuthEventType.TOKEN_REFRESHED, tokenRefreshSpy);
        authEventBus.on(AuthEventBus_1.AuthEventType.STATE_CHANGED, authStateChangeSpy);
    ...
    test('エラー処理と自動リカバリーフロー', async () => {
        ...
        authEventBus.on(AuthEventBus_1.AuthEventType.AUTH_ERROR, errorEventSpy);
        // 1回目は失敗、2回目は成功するシナリオ
        ...
        const result = await authService._verifyToken('test-token');
        // 検証 - 最終的に成功すべき
        ...
        // 検証失敗→リフレッシュ
        const resultAfterRefresh = await authService._verifyToken('expired-token');
        // 検証 - リフレッシュにより成功すべき
    ...
    test('ClaudeCode連携の動作確認', async () => {
        ...
        // ClaudeCode同期設定
        sinon.stub(claudeCodeSync, '_getClaudeConfigDir').returns('/mock/config/dir');
        // ログイン成功時のAPIレスポンスをモック
        mockAxios.post.withArgs('http://test-api.example.com/api/auth/login').resolves({
            status: 200,
            data: {
    ...
    test('セキュアなアクセス制御 - 認証ヘッダー生成', async () => {
        ...
        // 検証
        assert.deepStrictEqual(authHeader, { 'Authorization': 'Bearer secure-token-123' }, '正しい認証ヘッダーが生成されるべき');
        // トークンが変わった場合
        mockTokenManager.getAccessToken.resolves('updated-token-456');
        // 認証ヘッダーを再取得
        const updatedHeader = await authService.getAuthHeader();
        // 検証
        assert.deepStrictEqual(updatedHeader, { 'Authorization': 'Bearer updated-token-456' }, '最新のトークンで認証ヘッダーが生成されるべき');
        // トークンが存在しない場合
    ...
    test('認証エラーイベント連携とエラーリカバリー', async () => {
        ...
        assert.ok(authService.getLastError().code === 'invalid_credentials', 'エラーコードが設定されるべき');
        // レート制限エラーのシナリオ
        mockAxios.post.withArgs('http://test-api.example.com/api/auth/login')
        ...
        assert.strictEqual(lastError.code, 'rate_limited', 'レート制限エラーコードが設定されるべき');
        assert.strictEqual(lastError.isRetryable, true, 'リトライ可能フラグがtrueに設定されるべき');
        // 成功ケースのセットアップ
        mockAxios.post.withArgs('http://test-api.example.com/api/auth/login')
...
//# sourceMappingURL=authFlow.test.js.map